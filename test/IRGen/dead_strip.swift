...
// RUN: %target-swiftc_driver @%t-commonflags %s -I%t -L%t -lMyModule -Xlinker -dead_strip -module-name main -o %t/main-base
// RUN: %llvm-nm --defined-only %t/main-base | %FileCheck %s

// RUN: %target-swiftc_driver @%t-commonflags -emit-library -static -DMODULE -emit-module %s -module-name MyModule -Xfrontend -emit-dead-strippable-symbols -lto=llvm-full -Xfrontend -vtable-method-elimination -Xfrontend -wtable-method-elimination -Xfrontend -enable-associated-type-accessors
// RUN: %target-swiftc_driver @%t-commonflags %s -I%t -L%t -lMyModule -Xlinker -dead_strip -module-name main -o %t/main -lto=llvm-full -Xlinker -save-temps -save-temps -Xfrontend -emit-dead-strippable-symbols
// RUN: %llvm-dis %t/main.lto.bc
// RUN: %llvm-dis %t/main.lto.opt.bc
